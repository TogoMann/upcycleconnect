...
func (r *Repository) GetAll() ([]Course, error) {
	rows, err := r.db.Query(db.Ctx, "SELECT id, name, description, max_capacity, created_by, created_at, approved, approved_by, approved_at, price, date, start_time, end_time, status, correction_comment FROM course")
	if err != nil {
		return nil, err
	}
...
func (r *Repository) GetById(id pgtype.Int8) (*Course, error) {
	rows, err := r.db.Query(db.Ctx, "SELECT id, name, description, max_capacity, created_by, created_at, approved, approved_by, approved_at, price, date, start_time, end_time, status, correction_comment FROM course WHERE id = $1", id)
	if err != nil {
		return nil, err
	}
...
func (r *Repository) GetUserCourses(userId pgtype.Int8) ([]UserCourse, error) {
	rows, err := r.db.Query(db.Ctx, `
		SELECT c.id, c.name, c.description, c.max_capacity, c.created_by, c.created_at, c.approved, c.approved_by, c.approved_at, c.price, c.date, c.start_time, c.end_time, c.status, c.correction_comment, co.buyer_id, co.booked_at 
		FROM course c 
...
func (r *Repository) Update(id pgtype.Int8, c Course) error {
	_, err := r.db.Exec(db.Ctx, "UPDATE course SET name = $1, description = $2, max_capacity = $3, price = $4, date = $5, start_time = $6, end_time = $7, status = $8, correction_comment = $9 WHERE id = $10", c.Name, c.Description, c.MaxCapacity, c.Price, c.Date, c.StartTime, c.EndTime, c.Status, c.CorrectionComment, id)
	return err
}
