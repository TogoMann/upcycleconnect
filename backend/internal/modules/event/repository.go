...
func (r *Repository) GetAll() ([]Event, error) {
	rows, err := r.db.Query(db.Ctx, "SELECT id, approved, approved_by, approved_at, price, date, start_date, end_date, COALESCE(location, '') as location, created_by, created_at FROM event WHERE approved = true")
	if err != nil {
...
func (r *Repository) GetAllFull() ([]Event, error) {
	rows, err := r.db.Query(db.Ctx, "SELECT id, approved, approved_by, approved_at, price, date, start_date, end_date, COALESCE(location, '') as location, created_by, created_at FROM event")
	if err != nil {
...
func (r *Repository) GetById(id pgtype.Int8) (*Event, error) {
	rows, err := r.db.Query(db.Ctx, "SELECT id, approved, approved_by, approved_at, price, date, start_date, end_date, COALESCE(location, '') as location, created_by, created_at FROM event WHERE id = $1", id)
	if err != nil {
...
func (r *Repository) Create(dto Event) (pgtype.Int8, error) {
	...
	err := r.db.QueryRow(
		db.Ctx,
		"INSERT INTO event (approved, price, date, start_date, end_date, location, created_by) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id",
		dto.Approved, dto.Price, dto.Date, dto.StartDate, dto.EndDate, dto.Location, dto.CreatedBy).Scan(&id)
...
func (r *Repository) Update(id pgtype.Int8, e Event) error {
	tag, err := r.db.Exec(db.Ctx,
		"UPDATE event SET approved=$1, approved_by=$2, approved_at=$3, price=$4, date=$5, start_date=$6, end_date=$7, location=$8, created_by=$9 WHERE id=$10",
		e.Approved, e.ApprovedBy, e.ApprovedAt, e.Price, e.Date, e.StartDate, e.EndDate, e.Location, e.CreatedBy, id)
	if err != nil {
		return err
	}
	if tag.RowsAffected() == 0 {
