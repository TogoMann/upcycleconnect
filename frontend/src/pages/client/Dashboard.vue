...
const kpiScore = computed(() => clientStore.score)
const kpiAnnonces = computed(() => clientStore.annonces.filter((a: any) => a.status === 'active').length)
const kpiDepots = computed(() => clientStore.depots.length)
const kpiEntries = computed(() => clientStore.entries.length)
...
<template>
    <div class="dashboard">
        <TutorialOverlay v-if="showTutorial" @done="handleTutorialDone" />
        ...
        <div class="kpi-grid">
            <router-link to="/particulier/score" class="kpi-card kpi-card--link">
                <div class="kpi-icon">
...
.kpi-card--link:hover {
    ...
    box-shadow: 0 8px 24px rgba(8, 106, 53, 0.12);
}
.kpi-icon {
    width: 38px;
    height: 38px;
