...
const items = computed(() =>
    rawItems.value.map((item: any) => ({
        ...
        location: item.lieu || item.location || '',
    }))
)
...
const selectedEvents = computed(() =>
    selectedDate.value ? items.value.filter(e => e.date === selectedDate.value) : []
)
                        ...
                        <div class="day-cols">
                            ...
                            >
                                ...
                                >
                                    ...
                                    <span v-if="ev.location" class="wev-loc">{{ ev.location }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
...
.nav-btn {
    ...
    cursor: pointer; color: var(--charcoal); transition: all 0.15s;
}
...
.today-btn {
    padding: 7px 14px; border: 1.5px solid rgba(53,53,53,0.15); border-radius: 8px;
    background: none; font-size: 0.82rem; font-weight: 600; cursor: pointer;
    transition: all 0.15s; color: var(--charcoal); font-family: inherit;
}
.today-btn:hover { border-color: var(--green-mid); color: var(--green-dark); }

.view-toggle {
    display: flex; border: 1.5px solid rgba(53,53,53,0.15); border-radius: 8px; overflow: hidden;
}
.view-toggle button {
    padding: 7px 14px; background: none; border: none; font-size: 0.82rem;
    font-weight: 600; cursor: pointer; color: var(--charcoal); font-family: inherit; transition: all 0.15s;
}
...
.month-daynames {
    ...
    background: rgba(53,53,53,0.03); border-bottom: 1.5px solid rgba(53,53,53,0.08);
}
.dayname {
    text-align: center; font-size: 0.75rem; font-weight: 700;
    text-transform: uppercase; letter-spacing: 0.06em; padding: 10px 0; opacity: 0.5;
}
...
.event-chip {
    ...
    padding: 2px 5px; border-radius: 4px; font-size: 0.7rem; font-weight: 600;
...
.day-panel {
    ...
    border: 1.5px solid rgba(53,53,53,0.1); border-radius: 12px; overflow: hidden;
}
.day-panel-head {
    display: flex; align-items: center; justify-content: space-between;
    padding: 14px 18px; border-bottom: 1px solid rgba(53,53,53,0.08);
...
.day-ev-loc { opacity: 0.6; }

.week-view {
    background: var(--white); border: 1.5px solid rgba(53,53,53,0.1);
    border-radius: 14px; overflow: hidden;
}

.week-head {
    display: flex; border-bottom: 1.5px solid rgba(53,53,53,0.08); background: rgba(53,53,53,0.02);
}
...
.wt-label {
    font-size: 0.72rem; opacity: 0.4; text-align: right; padding-right: 10px;
    transform: translateY(-9px); display: flex; align-items: flex-start;
...
.week-ev {
    ...
    padding: 3px 6px; font-size: 0.72rem; overflow: hidden; z-index: 1;
...
.wev-loc { font-size: 0.65rem; opacity: 0.7; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
</style>
